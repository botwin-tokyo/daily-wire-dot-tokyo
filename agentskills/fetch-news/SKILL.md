...
To turn the freshly populated database into the site edition JSON, run:

```bash
npm run generate:edition
```

This reads `backend/db/news.db` and writes `public/data/current-edition.json` (plus dated archive and index files).
