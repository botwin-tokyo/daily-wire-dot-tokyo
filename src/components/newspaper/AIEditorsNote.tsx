  ...
  return (
    <section className="panel-tinted" aria-labelledby="ai-editors-note">
      <div className="flex items-center gap-2 border-b border-[var(--rule)] pb-2">
        <Star className="h-4 w-4" strokeWidth={1.6} style={{ color: "var(--accent-gold)" }} aria-hidden />
      ...
      {open && (
        <dl className="mt-3 grid grid-cols-2 gap-x-4 gap-y-1 border-t border-[var(--rule)] pt-3 text-[12px] font-sans text-[var(--ink-mid)]">
          <dt>Model</dt><dd className="font-mono">{n.model}</dd>
          <dt>Generated</dt><dd className="font-mono">{new Date(n.generatedAt).toLocaleString()}</dd>
          <dt>Sources considered</dt><dd className="font-mono">{n.sourcesConsidered}</dd>
          <dt className="col-span-2 italic text-[var(--ink-faint)] mt-1">Always consult original reporting for full context.</dt>
        </dl>
      )}
    </section>
  );
